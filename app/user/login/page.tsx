...
import { loginCustomer, clearError } from "@/lib/user/customerAuthSlice"
...
export default function CustomerLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!cardNo.trim() || !password.trim()) {
      return
    }

    try {
      await dispatch(loginCustomer({ cardNo: cardNo.trim(), password })).unwrap()
    } catch (error) {
  ...
  const handlecardNoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setcardNo(e.target.value.toUpperCase())
  }
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <div className="w-full flex justify-center mb-4">
            <Image src={logo} alt="Logo"  height={70} className="" />
          </div>
        ...
        <Card className="shadow-xl border-border bg-card">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full h-12 text-lg my-8 "
                disabled={isLoading || !cardNo.trim() || !password.trim()}
              >
                {isLoading ? (
                  <>
